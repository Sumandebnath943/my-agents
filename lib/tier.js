...
async function bump(provider, opts = {}) {
  ...
  // PGRST202 is PostgREST's "no such function" code, confirmed against the live project; the
  // message match is a backstop in case that code changes.
  const missing = rpcErr.code === "PGRST202" || /could not find|does not exist|schema cache/i.test(rpcErr.message || "");
