...
// Strip markdown to PLAIN TEXT — for places that render literal text (LinkedIn/social posts,
// which don't support markdown, so `**bold**` would show the asterisks).
...
// Convert an LLM markdown narrative to email-safe HTML — for narrative emails (team/brand
// reports) so they render as designed HTML instead of raw ** and #.
export function mdToHtml(md) {
  let h = esc(String(md ?? ""));
  h = h
    .replace(/^\s*#{3}\s+(.*)$/gm, '<div style="font-size:14px;font-weight:800;color:#1a1a2e;margin:14px 0 4px;">$1</div>')
    .replace(/^\s*#{2}\s+(.*)$/gm, '<div style="font-size:16px;font-weight:800;color:#1a1a2e;margin:16px 0 6px;">$1</div>')
    .replace(/^\s*#{1}\s+(.*)$/gm, '<div style="font-size:18px;font-weight:800;color:#1a1a2e;margin:16px 0 6px;">$1</div>')
    .replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>")
    .replace(/`([^`]+)`/g, '<code style="background:#f1eff0;padding:1px 5px;border-radius:4px;">$1</code>')
    .replace(/^\s*[-*+]\s+(.*)$/gm, '<div style="margin:3px 0 3px 6px;">• $1</div>')
    .replace(/^\s*\d+\.\s+(.*)$/gm, '<div style="margin:3px 0 3px 6px;">$1</div>');
  return h.replace(/\n/g, "<br>").replace(/(<\/div>)<br>/g, "$1").replace(/(<br>\s*){3,}/g, "<br><br>");
}
