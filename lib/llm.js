...
export function reason(status, body = "") {
  if (status === 402) return "quota_exhausted";
  if (status === 429) return QUOTA_RE.test(body) ? "quota_exhausted" : "rate_limit";
...
async function callOAI(provider, messages, opts = {}) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (!res.ok) {
      ...
      const why = reason(res.status, body);
    ...
    noteTokenWindow(provider, res, u.total_tokens ?? ((u.prompt_tokens || 0) + (u.completion_tokens || 0)));
...
async function _geminiDirect(prompt, opts = {}) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (!res.ok) {
      ...
      const why = reason(res.status, errBody);
