...
// Helicone observability (OPTIONAL). We route ONLY Gemini through Helicone's gateway — Gemini is our
// PUBLIC/visual lane, so private content (Groq: journal, finance, cover letters) is NEVER sent to a
// third-party observer. Fail-open: if the gateway is unreachable we retry the SAME call directly
// against Google, so enabling Helicone can never break a Gemini call — worst case it isn't logged.
const heliconeOn = () => !!process.env.HELICONE_API_KEY;
function geminiEndpoint(model, useHelicone) {
  const path = `/v1beta/models/${model}:generateContent?key=${env("GEMINI_API_KEY")}`;
  if (useHelicone) {
    return {
      url: `https://gateway.helicone.ai${path}`,
      headers: {
  ...
  return { url: `https://generativelanguage.googleapis.com${path}`, headers: { "Content-Type": "application/json" } };
}
...
export async function callGemini(prompt, opts = {}) {
  ...
  let useHelicone = heliconeOn();
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    await pace("gemini", model);
    const t0 = Date.now();
    const payload = JSON.stringify(body);
    let ep = geminiEndpoint(model, useHelicone);
    let res;
    try {
      res = await fetch(ep.url, { method: "POST", headers: ep.headers, body: payload });
    } catch (e) {
      if (!useHelicone) throw e;                 // direct failure -> real network error
      console.error("Helicone gateway unreachable, using Gemini direct:", e.message);
      useHelicone = false;                       // fail-open: retry this call directly
      ep = geminiEndpoint(model, false);
      res = await fetch(ep.url, { method: "POST", headers: ep.headers, body: payload });
    }
    const ms = Date.now() - t0;
    if (!res.ok) {
