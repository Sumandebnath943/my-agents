...
// 768-dim embedding via Gemini — same model ECHO uses. Google RETIRED text-embedding-004 (it 404s),
// which silently broke every memory write/recall, so we try the current model first and fall back
// through the older names, remembering whichever works. Pin one with GEMINI_EMBED_MODEL.
// 768 dims keeps new vectors the same size as the ones already stored.
export const EMBED_DIMS = 768;
const EMBED_MODELS = [process.env.GEMINI_EMBED_MODEL, "gemini-embedding-001", "text-embedding-004", "embedding-001"].filter(Boolean);
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
let _embedModel = null;

export async function embed(text) {
  const body = JSON.stringify({ content: { parts: [{ text: String(text).slice(0, 8000) }] }, outputDimensionality: EMBED_DIMS });
  const candidates = _embedModel ? [_embedModel] : EMBED_MODELS;
  const tried = [];
  for (const model of candidates) {
    for (let attempt = 0; attempt < 4; attempt++) {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:embedContent?key=${env("GEMINI_API_KEY")}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body,
      });
      if (res.ok) { _embedModel = model; return (await res.json()).embedding?.values || null; }
      if ((res.status === 429 || res.status >= 500) && attempt < 3) { await sleep(1500 * (attempt + 1)); continue; }
      tried.push(`${model}:${res.status}`);
      break; // a 4xx (e.g. 404 = retired model) won't fix itself — try the next candidate
    }
  }
  _embedModel = null;
  throw new Error(`embed failed — tried ${tried.join(", ")}. If all are 404, the embedding model was retired; set GEMINI_EMBED_MODEL to a current one.`);
}
