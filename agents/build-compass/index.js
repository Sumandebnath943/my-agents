...
// DEMAND RADAR: reads real problems people ask to have solved (Reddit + Ask HN) ->
// DECIDER: ranks YOUR idea backlog (#18 ideas table) against that demand and picks ONE thing
// to build next, with a copy-paste Claude Code prompt -> emails the call. Turns building from a
// guess into a response to demand.
...
// 1) DEMAND RADAR: what problems are people repeatedly asking to have solved?
const SUBS = ["SaaS", "webdev", "SideProject", "indiehackers", "automation"];
let posts = [];
for (const s of SUBS) {
  try { posts.push(...(await subredditNew(s, 25)).map((p) => ({ ...p, source: `r/${s}` }))); } catch {}
}
// Ask HN threads (people literally asking for tools/solutions).
const askHn = await fetch("https://hn.algolia.com/api/v1/search_by_date?tags=ask_hn&hitsPerPage=40")
  .then((r) => r.json()).catch(() => ({ hits: [] }));
posts.push(...(askHn.hits || []).map((h) => ({ title: h.title, text: "", url: `https://news.ycombinator.com/item?id=${h.objectID}`, source: "AskHN" })));
...
{"pick":"name","why":"2-3 sentences tying it to a demand signal","first_prompt":"a copy-paste Claude Code prompt to start building it","runner_up":"name"}.
